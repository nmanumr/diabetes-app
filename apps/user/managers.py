...
from diabetes.ext.soft_delete import SoftDeleteManager


class UserManager(SoftDeleteManager, AuthUserManager):
    def _create_user(self, username, email, *args, **kwargs):
        if not username:
            username = suggest_username(email)

        return super()._create_user(username, email, *args, **kwargs)
